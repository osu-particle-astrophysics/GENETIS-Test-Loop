...
Bicone_List = [6.12781, 39.2655, -0.00404388, 0.0344241, 5.99898, 68.8562, 0.00308239, -0.193231]
...
for i in range(0, len(r1)):
    R1_fit = abs(1-((r1[i] / Radius1)*1))
    R2_fit = abs(1-((r2[i] / Radius2)*1))
    L1_fit = abs(1-((l1[i] / Length1)*1))
    L2_fit = abs(1-((l2[i] / Length2)*1))
    T1_fit = abs(1-((t1[i] / Theta1) *1))
    T2_fit = abs(1-((t2[i] / Theta2) *1))
    b1_fit = abs(1-((b1[i] / B_coeff1) *1))
    b2_fit = abs(1-((b2[i] / B_coeff2) *1))

    fitness.append(100-100*((R1_fit+R2_fit+L1_fit+L2_fit+T1_fit+T2_fit+b1_fit+b2_fit)/8))
...
for i in range(0, len(fitness)): #introduce error
    if (fitness[i] == 0):
        ...
        error.append(25.0)
    fitness[i] = random.gauss(fitness[i], 25.0)
